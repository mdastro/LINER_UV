...
plotSFH=function(par, agemax=13.3, add=FALSE, col='black', ylim=NULL,...){
  magcurve(massfunc_exp(mSFR=10^par[1], 
                        mtau=10^par[2],
                        magemax=agemax),
...
genSED=ProSpectSED(massfunc=massfunc_exp,
                   mSFR=10^inpar[1],
                   mpeak=10^inpar[2],
                   mperiod=10^inpar[3],
                   mskew=inpar[4],
...
Data=list(flux=flux_input,
          arglist=list(massfunc=massfunc_exp, agemax=agemax, 
          ...
          #'CMA', or 'check')
          mon.names=c('z', 'LP','masstot','SFRburst',
